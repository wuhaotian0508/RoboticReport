...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt_file', default=None, help='text file with one prompt per line')
    parser.add_argument('--output_dir', default='out/conditional', help='directory for generated BVH files')
    ...
    if args.prompt_file:
        with open(args.prompt_file, 'r', encoding='utf-8') as f:
            prompts = [line.strip() for line in f if line.strip() and not line.lstrip().startswith('#')]
    else:
        prompts = [args.prompt or 'A person walking while talking on the phone.']

    for idx, text in enumerate(prompts):
        print(f'[{idx + 1}/{len(prompts)}] {text}')
        bert_feature, bert_mask = text2bert(text)
        trainer.evaluate_generate(text, idx, args.output_dir, agent_=agent, env_=env, clip_feature=None, bert_feature=bert_feature, bert_mask=bert_mask)
